...
In the first section the structure and functions of shiny apps will be explained. 
After that, some examples of shiny apps and their respective codes will be presented.
...
The last group of functions are the UI output functions. They create user interface elements that, in conjunction with server side rendering function, display different kinds of output from the application, like text, plots or tables. They are used in a similar manner to the input functions. Each output function therefore has the same first argument <code>outputId</code>, again representing an unique identifier string. These identifiers are then used in the server function like <code>output$my_output</code> paired with a <code>render()</code> function to process and display the output. The code below gives an example of how to integrate text, a plot and a table in the layout of the shiny app.
...
```{r slider, echo=FALSE,  out.width = '35%'}
knitr::include_graphics("img/slider.jpg")
```

#### Extra: Using Shiny in documents - interactive documents {.unlisted .unnumbered}
...
After installing and loading the shiny package the UI has to be defined. This includes the  layout and  the interactive elements. In this case  a dropdown menu and sliders.
The function <code>fluidPage()</code> is used to create a fluid layout which means that the layout is composed of rows and columns to keep all content in line. A fluid page layout is not fixed but scales its components depending on the window size.
After determining which layout should be used, the layout can be set.
...
<code>sidebarLayout()</code> defines that on the left side a sidebar can be created using the function <code>sidebarPanel()</code> and on the right side the main area which includes the visualization using the function <code>mainPanel()</code>. The sidebar includes a dropdown menu which is created with <code>selectInput()</code>. In this menu a distribution can be selected - normal, beta or t. Furthermore, the sidebar includes sliders where the values of the parameters can be chosen. 
...
The sliders are created with the function <code>conditionalPanel()</code>. Depending on which distribution was chosen in the dropdown menu above, the parameters and their value range vary. A deeper look on distributions and their density functions is given in [@mood1950introduction], were the following examples are taken from. A reference for other distributions like the _student_ $t$-distribution can be found in [@bronstein2012taschenbuch].

For the beta-distribution which is defined as follows:

\begin{align*}
& X \sim {\sf Beta}(\alpha, \beta) \\
& p(x|\alpha, \beta) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)\Gamma(\beta)}x^{\alpha -1}(1-x)^{\beta -1} \: \text{for} \: 0 < x < 1 \\
& \text{with} \: \alpha,\beta > 0
\end{align*}

the parameters are $\alpha$ and $\beta$. For the sliders we chose a value range between $1$ and $10$ for each parameter.

For the normal distribution

\begin{align*}
& X \sim \mathcal{N}\left( \mu, \sigma^2 \right) \\
& p(x|\mu, \sigma^2) = \frac{1}{\sqrt{2\pi\sigma^2}}exp\left\{-\frac{1}{2\sigma^2}(x-\mu)^2\right\} \: \text{for} \: x \in \mathbb{R} \\
& \text{with} \: \mu \in \mathbb{R} \: \text{and} \: \sigma^2 > 0
\end{align*}

the value of $\mu$ was chosen to be in the range between $-10$ and $10$ and the value of $\sigma$ in the range of $1$ to $10$.
...
For the $t$-distribution the degrees of freedom can be choosen from $1$ to $10$.
After creating the sidebar, the main panel has to be defined.
...
With the function <code>mainPanel(plotOutput("plot"))</code> a plot is created next to the sidebar. The plot varies depending on the chosen distribution.
Now that the UI is defined, the server logic has to be set.
...
The server logic is required to draw the distribution plots which are shown in the main panel. The plots are generated depending on the input given via the dropdown menu selection and the slider selection. 
The reactive plots are rendered using <code>rendersPlot()</code>.
Via an <code>if</code> condition the selection of the  distribution in the dropdown menu is considered. Depending on the selected distribution the different plots are created by determining the range of the $x$ axis and the type of distribution and considering the selected values for the parameters. Furthermore, the color of the graphs and the type of plot are set as well as the description of the $y$- and $x$-axis.
Now the UI and also the server logic are defined which means that the app can run. The app can be seen below.

```{r distr, echo=FALSE,  out.width = '100%'}
knitr::include_graphics("img/distribution1.jpg")
knitr::include_graphics("img/distribution2.jpg")
knitr::include_graphics("img/distribution3.jpg")
```
...
for one data set simultaneously. A formal introduction to linear regression models is given in [@fahrmeir2016statistik]. The results can be compared directly. Therefore the _Akaike information criterion (AIC)_ is calculated for each of the two models. The model specification can be changed at any time. So the AIC is an indicator, if the new specification yields a "better" model. Here, the model with the lowest AIC value is considered to be the best model. Another interesting view on model selection is provided by the plot in the app. This more machine-learning-like approach takes a partition of the data set, also called a test set, to calculate predictions with the fitted models. These predictions can be compared to the true test values. The train/test split rate can also be controlled in the app. This gives us some insight on how good we can make true predictions with each of the two models. Note that the models are fitted (trained) with the other partition of the initial data set, the so called training data set. In the following part of this section we will present the code and the app itself.

First, we need to include the packages for our interactive regression app. As an example data set we use the <code>BostonHousing</code> data set from the <code>mlbench</code> package.
...
The <code>BostonHousing</code> data set contains 506 census tracts of Boston from the 1970 census on 14 variables. The target being <code>medv</code> which represents the median value of owner-occupied homes in USD 1000's. 

```{r, echo=TRUE, R.options=list(width = 400)}
...
The complete app then has the following form.

```{r regression, echo=FALSE, out.width = '100%'}
...
The plot explained can be seen in the middle of the upper row in the "Plots"-tab. It shows the counties with the highest amount of cases per 100.000 inhabitants in the last 7 days. If the dropdown menu item "Germany (total)" is selected the counties with the top 5 highest values of entire Germany will be displayed, if a state is chosen the counties with the top 5 values within the state are displayed. The plot is again created by the <code>renderPlot()</code> function. Firstly, a character vector <code>xLabs</code> is created for the labels. Why this is needed will be explained a little later. Then the data is arranged in descending order of cases per 100.000 in the last 7 days and the top 5 counties are chosen. Then a bar plot is created using <code>geom_col()</code>. Because some cities are counties and have the same name as the county surrounding them (i.e., Bamberg city and Bamberg county) the type of county (city/not city/etc.) and the name of the county were pasted together to prevent inconsistent plotting and stacking of the data. This is why the <code>xLabs</code> object had to be created. It is used for the x-axis text and makes it easier to read by "reversing" the combination of both strings. The data was also ordered from highest to lowest inside of the plot using <code>reorder()</code>. The <code>scale_fill_distiller()</code> function was used to color the plot and scale the color with the value. The theme "classic" was set using <code>theme_classic()</code> and was chosen for aesthetic reasons. The x-axis text was rotated 90 degrees and a line break function for better readability using <code>angle</code> inside the <code>theme()</code> function and <code>str_wrap()</code> inside the <code>scale_x_discrete()</code> function. The legend and title were added in the <code>labs()</code> function. If a state is selected the creation of the plot is done equally except the data manipulation. Here the data is first filtered by the selected state using an <code>if</code> condition and <code>filter()</code>. The rest of the code remains the same except some strings for the legend.

As stated above, the other plots were all created in the same way except using other <code>arrange()</code> or <code>filter()</code> functions and different labels. The code for the other plots can be found in the source code of our [GitHub](https://github.com/SvenNekula/Shiny-Dashboards){target="_blank"}-repository or in the appendix at the end.

After explaining all the elements of the app in detail, the entire app can be seen below.

```{r cd, echo=FALSE, out.width = '100%'}
knitr::include_graphics("img/C19_dashboard1.jpg")
knitr::include_graphics("img/C19_dashboard2.jpg")
knitr::include_graphics("img/C19_dashboard3.jpg")
```

# Conclusion

Nowadays there is a need for clear visualization and interactions with large amounts of data in almost every area. Dashboards that use simple controls to set filters or restrict the display to a specific area of interest are particularly popular. With this solution, a wide range of users can be offered an intuitive presentation of the facts. In addition to the already mentioned purpose for the end user, the possibilities of creating such a tool in the programming language R presented in this paper offer further advantages. The R shiny package provides the developer a simple interface to create a link between to the classic statistical work and the summarizing of the results in a dashboard or a web app. The user interface and the server logic behind it can be implemented with a few functions. This can be done entirely within R, so no further knowledge of other web technologies is required. The examples in this document show how versatile R Shiny apps and dashboards can be. In addition to the usual control elements known from websites and computer programs, tables and plots can also be inserted. Even the integration of interactive maps can be done relatively easily. In the age of the web, R Shiny solutions represent an easily configurable and fast solution for preparing the core content of a topic for various target groups.

# Appendix

## Code of distribution plotter
...
## Code of interactive regression

```{r, eval=FALSE}
# regression_app.R
# In this script the Boston housing data set is used
# to predict the house price in Boston from house
