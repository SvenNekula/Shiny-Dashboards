...
the statistical capabilities of shiny. The final app is a Covid-19 dashboard to show
the visualization capabilities and the accessibility of data in shiny.
...
After installing and loading the shiny package the UI has to be defined. This includes the whole layout and in particular the interactive elements which in this case is a dropdown menu and sliders.
The function <code>fluidPage()</code> is used to create a fluid layout which means that the layout is composed of rows and columns to keep all content in line. A Fluid page layout is not fix but scale its components depending on the window size.
After determining the superior layout, the layout in particular can be set.
...
The sliders are created with the function <code>conditionalPanel()</code> - depending on which distribution was chosen in the dropdown menu above, the parameters and their value range are vary.
...
the parameters are $\alpha$ and $\beta$. For the sliders we chose a value range between $1$ and $10$ for each.
...
we decided that the value of $\mu$ can be chosen in the range between $-10$ and $10$ and the value of $\sigma$ in the range of $1$ to $10$.

The $t$-distribution is depending on the degrees of freedom $\nu$.
...
For the $t$-distribution the number of the degrees of freedom can be choosen in a range from $1$ to $10$.
...
With the function <code>mainPanel(plotOutput("plot"))</code> a place for the visualisation of each distribution depending on the chosen justifications is created next to the sidebar. 
Now that the UI is completely defined, the server logic has to be set.
...
The server logic is required to draw the distribution plots that shall be shown in the main panel. The plots are generated depending on the input given via the dropdown menu selection and the slider selection. 
The reactive plots are rendered using <code>rendersPlot()</code>.
Via an <code>if</code> condition the selection of the kind of distribution in the dropdown menu is considered. Depending on the selected distribution the particular plots are created by determining the range of the $x$ axis and the kind of distribution and considering the selected values for the parameters. Furthermore, the colors of the graphs is set and that lines are used to plot the data as well as the description of the $y$- and $x$-axis.
Now the UI and also the server logic are defined which means that the app can run. The app can be seen down below.
